  ...
  return (
    <aside className="space-y-8">
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-2">
            <input 
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-white/10 border border-white/20 text-white placeholder:text-blue-200 focus:outline-hidden focus:bg-white/20 transition-all text-sm"
            />
            <button className="w-full py-2.5 rounded-xl bg-white text-blue-600 font-bold text-sm hover:bg-blue-50 transition-colors">
              Langganan
            </button>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {trendingArticles.map((article, idx) => (
            <div key={article.id} className="group flex items-start gap-4">
              ...
              <div>
                <span className="mb-1 block text-xs font-bold uppercase text-emerald-600">
                  {(article.categories as Category[])?.[0]?.name || 'Umum'}
                </span>
                <Link 
                  href={`/artikel/${article.slug}`}
                  className="mb-2 block font-medium leading-snug text-slate-900 transition-colors group-hover:text-emerald-600 dark:text-white dark:group-hover:text-emerald-400"
                >
                  {article.title}
                </Link>
                {article.readingTime && (
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <Link
              ...
              className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-600 transition-colors hover:bg-emerald-50 hover:text-emerald-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400"
            >
              #{tag.name}
            </Link>
          ))}
      ...
      <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-600 p-6 text-white shadow-lg">
        ...
        <p className="mb-4 text-sm text-emerald-50 opacity-90">
